...
export default function CreateBlogPopup({ visible, onClose, onSubmit }) {
  ...
  const handlePost = async () => {
    ...
    setLoadingPost(true);
    try {
      await onSubmit({ title, content, image: images });
      setTitle("");
      setContent("");
      setImages([]);
    } catch (err) {
      let errorMessage = "Lỗi không xác định khi đăng bài viết.";
      if (err?.response?.data?.message) {
        errorMessage = err.response.data.message;
      } else if (err?.message) {
        errorMessage = err.message;
      }
      console.error("Lỗi đăng:", errorMessage);
      alert(errorMessage);
    } finally {
